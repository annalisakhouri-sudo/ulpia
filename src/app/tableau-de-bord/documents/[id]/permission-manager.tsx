...
export function PermissionManager({
  ...
  function handleGrant() {
    ...
    startTransition(async () => {
      await grantPermission(documentId, selected);
      setSelected("");
    });
  }
  ...
  function handleRevoke(permissionId: string) {
    startTransition(async () => {
      await revokePermission(permissionId, documentId);
    });
  }

  function copyLink(token: string, permissionId: string) {
    navigator.clipboard.writeText(`${siteUrl}/acces/${token}`);
  ...
  return (
    <div className="bg-surface border border-line rounded-lg p-6">
      <h2 className="font-display text-lg mb-4">Accès accordés</h2>

      {permissions.length === 0 ? (
        <p className="text-sm text-ink-soft mb-4">
          Personne n&apos;a encore accès à ce document.
        </p>
      ) : (
